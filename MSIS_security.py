...
if __name__ == '__main__':
    ...
    d = 128  # 多项式环的维度
    alpha = 1
    s = alpha * tau * eta * math.sqrt(k*d)
    B = s * math.sqrt(2*m*d) # ||z|| < B
    norm = math.sqrt(B**2 + tau)  # [s | bc]的二阶范数
    q = 2 ** 32 - 99 # 模数q

    print("norm: ", norm)

    # 参数设置
    ps = MSISParameterSet(d, k, 1, norm, q, "l2")
    ...
    cp = svp_classical(x[0])
    qp = svp_quantum(x[0])

    print("svp_classical: ", cp)
    print("svp_quantum: ", qp)



    # 困难问题 [A1 | A2] [ cz1-c'z1 | cz2-c'z2]^T = 0
    # d = 128  # 多项式环的维度
    # q = 2 ** 48 # 模数q
    #
    # nu = 0
    ...
    # # 标准差 s1 和 s2
    # s1 = alpha1 * tau * Bs1
    # s2 = alpha2 * tau * eta2 * math.sqrt(m2 * d)
    #
    # B1 = s1 * math.sqrt(2*(m1+1)*d)
    # B2 = s2 * math.sqrt(2*m2*d)
    # B = 8 * tau * math.sqrt(B1*B1 + B2*B2)
    # print("B1: ", B1)
    # print("B2: ", B2)
    # print("B: ", B)
    #
    # ps2 = MSISParameterSet(d, (m1+m2+1), 1, B, q, "l2")
    #
    # x2 = MSIS_summarize_attacks(ps2)
    #
    # delta2 = delta_BKZ(x2[0])
    #
    # print(delta2)
